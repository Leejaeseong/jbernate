...
public class NumUtil {
	 ...
	 * 				else	: 원래 값, 소수점 아래는 자름 
	 */
	public static BigDecimal bigDecimalNumVal( Object str ) {
		if( str == null || str.toString().equals( "" ) ) 	return new BigDecimal( 0 );
		else {
			if( str.toString().indexOf( "." ) != -1 ) {
					return new BigDecimal( str.toString().substring( 0, str.toString().indexOf( "." ) ) );
			}else	return new BigDecimal( str.toString() );
		}
	}
